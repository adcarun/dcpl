...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
	<head>
        ...
        <title>VI Schools | Duplicate Enquries</title>
            ...
            <div id="inner-container">
                ...
                <form name="frm" method="post">
                    ...
                    <?php  
						...
						$cond.=" group by Email having count(*) >= 2";
						set_management("enquiry_details", $cond, "Email", "desc", 15);
						
						$toPass=str_replace('Where'," ",$cond)."  ORDER BY ".$sortbyColumn." ".$sortbyOrder." LIMIT ".($page_size*($page_no-1) .  ", " . $page_size);
						//echo $toPass;
						$arrData=$obj->getDataRestricted("count(*) as TotalRec,Email",$toPass);
						
						$objTotalEnq=new GenericClass("enquiry_details");
						$arrTotal=$objTotalEnq->getDatalimited("count(*) as TotalRec"," group by Email having count(*) >= 2 ",false);
                    ...
                    <ul id="nav-info" class="clearfix">
                        ...
                        <li class="active"><a href="javascript:void(0)">Duplicate Enquiry</a></li>
                    </ul>
                    <!-- END Navigation info -->

                    <!-- Row Classes -->
                    <h3 class="page-header">Duplicate Enquiry</h3>
                    	<h5 class="form-box-header">Note :<br>
							<ul class="icons-ul">
                            	<li><i class="icon-li icon-arrow-right text-black"></i><small>Here administrator can view Duplicate Enquiry.</small></li>
                            </ul>
                        </h5>
                    ...
                    <?php
						...
						$arrTotal=$objTotalEnq->getDatalimited("count(*) as TotalRec,Email"," where Email!=''  group by Email having count(*) >= 2 ",false);	
						}else{
						$arrTotal=$objTotalEnq->getDatalimited("count(*) as TotalRec,Email"," where Email!=''  and AssignedTo=".$_SESSION['objLogin']->Id." group by Email having count(*) >= 2 ",false);
						}
						foreach($arrTotal as $rectotal)
						{
							$RTotal = $RTotal+$rectotal['TotalRec'];
                    ...
                    <table id="example-datatables" class="table table-striped table-bordered table-hover">
						...
						foreach($arrTotal as $rectotal)
						{
                            ...
                            <tr>
                                ...
                                <td><a href="./view-duplicate-enquirers.php?Email=<?=$rectotal['Email']?>&I=14" data-toggle="tooltip" title="View Details"><?=htmlspecialchars_decode(htmlspecialchars_decode(stripslashes($rectotal['Email'])));?></a></td>
                ...
                <?php
					$objTotalEnq=new GenericClass("enquiry_details");
					if($_SESSION['objLogin']->AccessLevel=="Admin"){
					$arrTotal=$objTotalEnq->getDatalimited("count(*) as TotalRec,Email"," group by Email having count(*) >= 2 ",false);
					}else{
					$arrTotal=$objTotalEnq->getDatalimited("count(*) as TotalRec,Email"," where AssignedTo=".$_SESSION['objLogin']->Id." group by Email having count(*) >= 2 ",false);	
					}
					foreach($arrTotal as $rectotal)
					{
						$RTotal = $RTotal+$rectotal['TotalRec'];
