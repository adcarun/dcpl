                ...
                <form name="frm" method="post">
					...
					<div class="price-tables">
                        ...
                        <?php
                            ...
                            $arrExecutive=$objExecutive->getDatalimited("Id,Name"," where Id = ".$_SESSION['objLogin']->Id,false);
                                                    	...
                                                    	<li>
                                                            <a href="javascript:ViewExecutiveEnquiryDetails('<?=$Executive['Id']?>','<?=$SiteCodes['Id']?>','./manage-enquiry.php?I=4');" data-toggle="tooltip" title="Click to view details">
                                                            	<span class="badge badge-<?=$class?>">
                                                                	<?php
																	if($_SESSION['objLogin']->AccessLevel=="Admin"){
																		$ArrCnt=$objTotalCnt->getDatalimited("COUNT(Id) AS cnt"," WHERE Id!=0 and AssignedTo = ".$Executive['Id']." AND Status = '".$SiteCodes['Id']."' ",false);
																	}else{
                                                                  //  $ArrCnt=$objTotalCnt->getDatalimited("COUNT(Id) AS cnt"," WHERE Id!=0 and AssignedTo = ".$Executive['Id']." AND Status = '".$SiteCodes['Id']."' and Type!=38 ",false);
																  $ArrCnt=$objTotalCnt->getDatalimited("COUNT(Id) AS cnt"," WHERE Id!=0 and AssignedTo = ".$Executive['Id']." AND Status = '".$SiteCodes['Id']."' and (Type IS NULL or Type!=38) ",false);
																	}
																	//echo "<br/>";
