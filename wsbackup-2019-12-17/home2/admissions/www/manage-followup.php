...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        ...
        <title>VI Schools | Manage Follow Up</title>
                ...
                <form name="frm" method="post">
                    ...
                    <?php 
						if($_SESSION['objLogin']->AccessLevel=="Admin"){
							$cond=" Where Id != 0 and NextUpdateDate >= '".date('Y-m-d')."' and EnqId IN(select Id from tbl_enquiry)"; 
						}
						else
						{
							$cond=" Where Id != 0 and NextUpdateDate >= '".date('Y-m-d')."' and UserId = ".$_SESSION['objLogin']->Id." and EnqId IN(select Id from tbl_enquiry)"; 	
						}
						if($_POST['srch_Feed_From_Date']!="")
							$cond.=" AND NextUpdateDate >= '".htmlspecialchars(addslashes(trim($_POST['srch_Feed_From_Date'])))."' ";
							
						if($_POST['srch_Feed_To_Date']!="")
							$cond.=" AND NextUpdateDate <= '".htmlspecialchars(addslashes(trim($_POST['srch_Feed_To_Date'])))."' ";
						
						if($_POST['srch_attendedDate']!="")
							$cond.=" AND RegTime  like '%".htmlspecialchars(addslashes(trim($_POST['srch_attendedDate'])))."%' ";
						...
						set_management("tbl_follow_up", $cond, "NextUpdateDate", "ASC", 15);
                    ...
                    <ul id="nav-info" class="clearfix">
                        ...
                        <li class="active"><a href="javascript:void(0)">Manage Follow Up</a></li>
                    </ul>
                    <!-- END Navigation info -->

                    <!-- Row Classes -->
                    <h3 class="page-header">Manage Follow Up</h3>
                    	<h5 class="form-box-header">Note :<br>
							<ul class="icons-ul">
								...
								Here administrator can view Manage Follow Up.
								<?php }else{ ?>
									Here Executive can view Manage Follow Up.
								<?php } ?>
								</small></li>
                            </ul>
                        </h5>
					...
					<div class="row" style="padding:0 0 10px 0">
						...
						<div class="col-md-4">
                                <div class="col-md-5">
                                <label class="control-label" for="example-input-small">Follow up Date (From) :</label>
                                </div>
                                <div class="col-md-7">
                        ...
                        <div class="col-md-4">
                        	<div class="col-md-5">
                        		<label for="example-select" class="control-label">Follow up Date (To) :</label>
                        	</div>
                        	<div class="col-md-7">
                        		<div class="input-group date input-datepicker " data-date="<?=date('d/m/Y');?>" data-date-format="yyyy-mm-dd">
                            		<span class="input-group-addon"><i class="icon-calendar icon-fixed-width"></i></span>
						...
						<div class="col-md-4">
                        	<div class="col-md-5">
                        		<label for="example-select" class="control-label">Attended Date :</label>
                        	</div>
                        	<div class="col-md-7">
                        		<div class="input-group date input-datepicker " data-date="<?=date('d/m/Y');?>" data-date-format="yyyy-mm-dd">
                            		<span class="input-group-addon"><i class="icon-calendar icon-fixed-width"></i></span>
                                	<input type="text" id="AttenedDate" name="srch_attendedDate" value="<?=htmlspecialchars_decode(htmlspecialchars_decode(stripslashes($_POST['srch_attendedDate'])));?>" class="form-control" readonly style="width:110px;" placeholder="Attended On" >
                            	</div>
                        	</div>
                        </div>
                        ...
                        <div class="col-md-4">
                            <div class="col-md-12">
                                ...
                                <button class="btn btn-success" type="button" onClick="clearSearch(this.form, ['srch_AssignedTo','TransactionDateFrom','TransactionDateTo','srch_attendedDate']);">Show all</button>
                            </div>
                        </div>
                     </div>     
                  </div>
                  <?php }else{ ?>    
					...
					<div class="row" style="padding:0 0 10px 0">
                        ...
                        <div class="col-md-4">
                            <div class="col-md-12">
                                <button class="btn btn-success" type="submit" onClick="return checkFields(this);">Search</button>
                                <button class="btn btn-success" type="button" onClick="clearSearch(this.form, ['TransactionDateFrom','TransactionDateTo','AttenedDate']);">Show all</button>
                            </div>
                        </div>
                     </div>     
                  </div>
                        ...
                        <tbody>
							...
							?>
                                    <tr class="<?=$class?>">
										...
										<td class=""><a href="javascript:EditRecord('<?= $arrEnquiry[0]['Id'] ?>', './view-enquiry.php?I=4&ref=manage');" data-toggle="tooltip" title="View Enquiry" ><?=$arrEnquiry[0]['Name'];?></a></td>
                                        ...
                                        <td class="">
                                            <div class="btn-group">
                                            	<a href="javascript:EditRecord('<?= $arrData[$key]['EnqId'] ?>', './enquiry-follow.php?I=12&ref=manage');" data-toggle="tooltip" title="Follow up" class="btn btn-xs btn-primary"><i class="icon-book"></i></a>
												<?php if($_SESSION['objLogin']->AccessLevel=="Admin"){ ?>
												<a href="javascript:EditRecord('<?= $arrData[$key]['Id'] ?>', './edit-followup.php?I=12&ref=manage');" data-toggle="tooltip" title="Edit Follow up" class="btn btn-xs btn-success"><i class="icon-pencil"></i></a>
												<?php } ?>
                                            </div>
                                        </td>
                                        <?php //} ?>
