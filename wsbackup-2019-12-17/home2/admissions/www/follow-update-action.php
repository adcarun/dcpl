<?php
	...
	if($_POST['TodaysUpdate'] != "")
	{
		...
		//exit();
			 $insertSql = "update tbl_follow_up set TodaysUpdate='".htmlspecialchars(addslashes(trim($_POST['TodaysUpdate'])))."', NextUpdateDate='".htmlspecialchars(addslashes(trim($_POST['NextUpdateDate'])))."', File='".htmlspecialchars(addslashes(trim($_POST['File'])))."', RegTime='".$_POST['RegTime']."' where Id=".htmlspecialchars(addslashes(trim($_POST['Id']))); 
			//exit();
			$res = mysql_query($insertSql);
			$_SESSION['Sucess'] = "Follow Up Details Edited successfully";
			
		//$insertSql = "insert into tbl_follow_up (EnqId,UserId,TodaysUpdate,NextUpdateDate,File,RegTime) values ('".htmlspecialchars(addslashes(trim($_POST['Id'])))."','".$_SESSION['objLogin']->Id."','".htmlspecialchars(addslashes(trim($_POST['TodaysUpdate'])))."','".htmlspecialchars(addslashes(trim($_POST['NextUpdateDate'])))."','".htmlspecialchars(addslashes(trim($_POST['File'])))."','".$_POST['RegTime']."')";
		//$res = mysql_query($insertSql);
...
<html>
    <body>
    <form name="frm" method="post" action="./manage-followup.php?I=12">
